    ...
    for it in range(1, n_iter + 1):
        ...
        try:
            ...
            if auc is not None and auc > best_auc:
                ...
                # Save val preds + ground truth for meta-agent ensemble phase
                try:
                    _best_preds = _slot_predict(slot, X_train, y_train, X_val, y_val)
                    np.save(str(LOGS_DIR / "best_val_preds.npy"), _best_preds)
                    np.save(str(LOGS_DIR / "y_val.npy"), y_val)
                    import json as _json
                    (LOGS_DIR / "best_auc.json").write_text(
                        _json.dumps({"auc": best_auc, "n_scored": n_scored}), encoding="utf-8"
                    )
                except Exception as _e:
                    print(f"  [meta] Could not save val preds: {_e}")
