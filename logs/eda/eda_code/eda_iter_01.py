...
print("SECTION 5 — MULTI-LABEL")

train = _load_csv(TRAIN_CSV)
if not train:
    print("No data in train.csv")
else:
    has_secondary = sum(1 for r in train if _parse_secondary_labels_row(r))
    total_rows = len(train)
    ratio_with_secondary = has_secondary / total_rows if total_rows > 0 else 0
    print(f"Rows with secondary labels: {has_secondary} ({ratio_with_secondary:.2%})")